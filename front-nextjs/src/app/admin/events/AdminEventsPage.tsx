...
function SummaryCard({
	...
	return (
		<div className="min-w-0 rounded-2xl border border-zinc-800 bg-zinc-950/70 p-3 sm:p-4">
			<p className="break-words text-[10px] uppercase tracking-[0.18em] text-zinc-500 [overflow-wrap:anywhere] sm:text-[11px] sm:tracking-[0.22em]">
				{label}
			</p>
			<p
				className={`mt-2 break-words text-xl font-semibold [overflow-wrap:anywhere] sm:mt-3 sm:text-2xl ${accentClass}`}
			>
				{value}
			</p>
		</div>
...
function EventMetric({
	...
	return (
		<div className="min-w-0 rounded-2xl border border-zinc-800 bg-zinc-950/80 px-4 py-3">
			<p className="break-words text-[11px] uppercase tracking-[0.22em] text-zinc-500 [overflow-wrap:anywhere]">
				{label}
			</p>
			<p className="mt-2 break-words text-sm font-medium text-zinc-100 [overflow-wrap:anywhere]">
				{value}
			</p>
		</div>
	)
...
function EventCard({ event }: { event: ManagedEventSummary }) {
	return (
		<Link
			...
			className="group block min-w-0 rounded-3xl border border-zinc-800 bg-zinc-900/70 text-white transition-all hover:border-zinc-700 hover:bg-zinc-900"
		>
			<div className="p-5 sm:p-6">
				<div className="flex min-w-0 flex-col gap-4 md:flex-row md:items-start md:justify-between">
					<div className="min-w-0">
						<p
							...
							className="max-w-full truncate text-[11px] uppercase tracking-[0.24em] text-zinc-500"
						>
							{event.organization.name}
						</p>
						<h2
							title={event.title}
							className="mt-3 line-clamp-2 break-words text-xl font-semibold text-zinc-50 transition-colors [overflow-wrap:anywhere] group-hover:text-white sm:text-2xl"
						>
							{event.title}
						</h2>
					</div>
				...
				<div className="mt-4 flex min-w-0 flex-wrap gap-2 text-sm text-zinc-400">
					<span className="max-w-full truncate rounded-full border border-zinc-800 bg-zinc-950/70 px-3 py-1.5">
						{event.type}
					</span>
					<span className="max-w-full truncate rounded-full border border-zinc-800 bg-zinc-950/70 px-3 py-1.5">
						{event.format}
				...
				<div className="mt-6 grid gap-3 md:grid-cols-3">
					...
					<EventMetric
						label="Завершение"
						value={formatDate(event.dataEnd)}
					/>
					<EventMetric
...
export function AdminEventsPage() {
	...
	const privateCount = events.filter(event => event.status === 'PRIVATE').length
		...
		<div className="grid min-w-0 w-full gap-6 text-white">
				...
				<div className="p-4 sm:p-5 md:p-7 xl:p-8">
					<div className="flex min-w-0 flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
						<div className="min-w-0 max-w-5xl">
							...
							<h1 className="mt-3 break-words text-3xl font-bold tracking-tight text-zinc-50 [overflow-wrap:anywhere] sm:text-4xl">
								Мои мероприятия
							</h1>
							<p className="mt-4 max-w-4xl break-words text-sm leading-6 text-zinc-400 [overflow-wrap:anywhere]">
								Здесь собраны все мероприятия, к которым у вас есть доступ как
								у администратора. Выберите нужное событие, чтобы перейти к
			...
			{filteredEvents.length ? (
				...
				<div className="rounded-[2rem] border border-zinc-800 bg-zinc-900/60 p-8">
					...
					<h2 className="mt-3 break-words text-2xl font-semibold text-zinc-100 [overflow-wrap:anywhere]">
						По выбранному фильтру мероприятий нет
					</h2>
					<p className="mt-3 max-w-2xl break-words text-sm leading-6 text-zinc-400 [overflow-wrap:anywhere]">

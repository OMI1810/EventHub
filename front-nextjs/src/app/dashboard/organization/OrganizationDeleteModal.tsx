...
export function OrganizationDeleteModal({ onClose }: Props) {
	const router = useRouter()

	const { mutate: mutateDeleteOrganization, isPending } = useMutation({
		mutationKey: ['organization', 'delete-account'],
		mutationFn: () => organizationService.deleteMyOrganizationAccount(),
		async onSuccess() {
			try {
			...
			toast.success('Аккаунт организации удалён')
			router.push(PUBLIC_PAGES.LOGIN)
		},
		onError() {
			toast.error('Не удалось удалить аккаунт организации')
		}
	})
			...
			<div className="w-full max-w-lg rounded-3xl border border-zinc-800 bg-zinc-900 p-6 text-white shadow-2xl">
				...
				<div className="mt-6 rounded-2xl border border-rose-900/60 bg-rose-950/30 px-5 py-5">
					<p className="text-sm leading-6 text-zinc-200">
						Вы точно хотите удалить аккаунт организации? Это действие удалит
						аккаунт владельца, саму организацию, связанных администраторов,
						заявки на вступление и все связанные с организацией данные.
					</p>
				</div>
				...
				<div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end">
					...
					<button
						type="button"
						onClick={() => mutateDeleteOrganization()}
						disabled={isPending}

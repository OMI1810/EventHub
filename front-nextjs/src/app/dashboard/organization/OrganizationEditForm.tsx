...
const ArcGisPointMap = dynamic(
		...
		),
	{
		ssr: false,
		loading: () => (
			<div className="h-72 max-w-full rounded-md border border-zinc-700 bg-zinc-950" />
		)
	}
)
...
const fieldClassName =
	'mt-2 w-full min-w-0 max-w-full rounded-xl border border-zinc-700 bg-zinc-900 px-4 py-3 text-sm text-white outline-none transition-colors focus:border-emerald-500'
...
export function OrganizationEditForm({ onCancel, organization }: Props) {
	...
	return (
		<form
			...
			className="mt-6 max-w-full space-y-4 overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950/70 p-4 sm:p-5"
		>
			<div className="grid min-w-0 gap-4 sm:grid-cols-2">
				<label className="min-w-0 text-sm text-zinc-300">
					Название организации
					<input type="text" {...register('name')} className={fieldClassName} />
				</label>
				...
				<label className="min-w-0 text-sm text-zinc-300 sm:col-span-2">
				...
				<div className="min-w-0 sm:col-span-2">
					<AddressAutocomplete
						label="Адрес"
						value={addressValue}
						labelClassName="relative grid min-w-0 gap-2 text-sm text-zinc-300"
						...
						onSelect={address => {
							...
							setValue('cordinatX', address.cordinatX, {
								shouldDirty: true
							})
							setValue('cordinatY', address.cordinatY, {
								shouldDirty: true
							})
							setOrganizationCoordinates({
			...
			<div className="flex min-w-0 flex-col gap-3 pt-2 sm:flex-row sm:justify-end">
				<button
					...
					onClick={onCancel}
					disabled={isPending}
					className="w-full rounded-xl border border-zinc-700 px-5 py-2.5 text-sm font-medium text-zinc-200 transition-colors hover:bg-zinc-800 disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
				>
					Отмена
				</button>
				...
				<button
					type="submit"
					disabled={isPending}
					className="w-full rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-emerald-500 disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
				>
